...
function setup_theme_admin_menus() {  
   add_submenu_page( 'themes.php', "Theme Settings", 'Theme Settings', 'manage_options', 'theme-settings', 'theme_settings' );
}
...
function theme_settings() {
   ...
   if( isset( $_POST["update_settings"] ) ) {
      ...
      if( $global_config->use_email === TRUE ) {
         $email_address = esc_attr( filter_input( INPUT_POST, "email_address" ) );
      ...
      foreach( $global_config->social_channels as $channelName ) {
         update_option( "social_".$channelName, esc_attr( filter_input( INPUT_POST, "social_".$channelName ) ) );
      }
...
function save_homepage() {
   ...
   for( $i = 1; $i <= $global_config->homepagePanels; $i++ ) {
      update_post_meta( $post->ID, 'home_panel_'.$i, filter_input( INPUT_POST, 'home_panel_'.$i ) );
   }
//   for($i = 1; $i <= $global_config->banner_count; $i++) {
